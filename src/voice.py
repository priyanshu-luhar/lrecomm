...
# === Logging ===
LOGFILE = os.path.join("../logs", "audio.log")
...
class ReticulumTelephone():
    ...
    def __init__(self, identity, owner = None, service = False, speaker=None, microphone=None, ringer=None, auto_answer=False):
        ...
        logging.info(f"{self} initialised")
    ...
    def status_text(self):
        if self.is_in_call:
            return f"In Call with {RNS.prettyhexrep(self.caller.hash)}"
        elif self.is_ringing:
            return f"Incoming call from {RNS.prettyhexrep(self.caller.hash)}"
        elif self.call_is_connecting:
            return f"Calling {RNS.prettyhexrep(self.caller.hash)}"
        else:
            return "Idle"
    ...
    def dial(self, identity_hash):
        ...
        logging.info(f"Path to {RNS.prettyhexrep(destination_hash)} exists: {has_path}")   
        ...
        if RNS.Transport.has_path(destination_hash):
            ...
            logging.info(f"Connecting call over {call_hops} hop{cs}...")
    ...
    def call_ended(self, remote_identity):
        if self.is_in_call or self.is_ringing or self.call_is_connecting:
            if self.is_in_call:
                ...
                logging.info(f"Call with {RNS.prettyhexrep(self.caller.hash)} ended")
            if self.is_ringing:         
                RNS.log(f"Call {self.direction} {RNS.prettyhexrep(self.caller.hash)} was not answered\n", RNS.LOG_DEBUG)
                logging.info(f"Call {self.direction} {RNS.prettyhexrep(self.caller.hash)} was not answered")
            if self.call_is_connecting: 
                RNS.log(f"Call to {RNS.prettyhexrep(self.caller.hash)} could not be connected\n", RNS.LOG_DEBUG)
