...
def main():
    ...
    parser.add_argument(
        ...
        help="Output WAV file path for recorded sink"
    )
    ...
    file_sink = FileSink(args.output)

    # Codec and pipeline setup
    if args.codec == "raw":
        codec = LXST.Codecs.Raw()
    elif args.codec == "codec2":
        codec = LXST.Codecs.Codec2(mode=LXST.Codecs.Codec2.CODEC2_700C)
    ...
    try:
        ...
        output_pipeline = LXST.Pipeline(source=loopback, codec=codec, sink=file_sink)
        ...
        file_sink.write_wav()
        time.sleep(1)
        print(f"[INFO] Pipeline stopped. WAV written to {args.output}. Exiting.")
